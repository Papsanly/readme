...
function languageSection(language: string | undefined): string {
  ...
  return [
    ...
    'pronounce the term. Common Latin → Ukrainian mistakes to avoid:',
    '- Latin **X** is pronounced **"кс"** (e.g. EXA → "екса"), NOT "х"',
    '- Latin **C** is **"к"** or **"ц"** depending on context, NOT "с"',
    '- Latin **H** is usually **"г"** (or silent depending on origin), NOT "н"',
    '- Latin **G** is **"ґ"** (hard) or **"г"** (soft), NOT "дж"',
    '- Latin **W** is **"в"**, NOT "ш"',
    '- Latin **N** is **"н"**, NOT "и"',
    '- Latin **Y** is **"ай"**, **"і"** or **"й"** depending on origin, NOT "у"',
    '',
    '**Use only standard target-alphabet letters**. For Ukrainian use the Ukrainian',
    'Cyrillic alphabet (А-Я, а-я, including Є/є, Ї/ї, Ґ/ґ, І/і). Do not use letters',
    'from other Cyrillic variants (Ё, Ы, Э, Ъ, Ў, etc.) even if they look phonetically',
    'close.',
    '',
    ...
    '',
    'Examples — dominant language **Ukrainian**:',
    '- Word-style (no hyphens): NATO → "нато"; NASA → "наса"; AJAX → "аякс"; EXA → "екса".',
    '- Letter-style (hyphens): USB → "ю-ес-бі"; PDF → "пі-ді-еф"; FBI → "еф-бі-ай"; HTML → "ейч-ті-ем-ел".',
    '- Proper nouns: Apple → "епл"; Microsoft → "майкрософт"; iPhone → "айфон"; Google → "ґуґл".',
    '- Diagram with English UI labels → describe in Ukrainian, not in English.',
    '',
    '**WRONG examples to avoid**:',
    '- ❌ "програма EXA" (Latin in `text`) — must be "програма екса"',
    '- ❌ "формат PDF" (Latin in `text`) — must be "формат пі-ді-еф"',
    '- ❌ "э-к-с-а" or "ЭКСА" (wrong alphabet — Э is not part of the Ukrainian alphabet)',
    '- ❌ "е-к-с-а" (over-hyphenated word that should be a single word "екса")',
    '- ❌ "Mixed латиниця and кирилиця in one word"',
    '',
    'Examples — dominant language **English**:',
    '- "the city of Kyiv", `rawText`: "the city of Київ"',
    '- "the company Monobank", `rawText`: "the company Монобанк"',
    '- A diagram with French/Spanish/Ukrainian labels → describe in English.',
    '',
