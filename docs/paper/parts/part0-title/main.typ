...
#let title-main() = [
  ...
  #align(center)[
    #text(size: 20pt, weight: "bold")[Дипломний проєкт]\
  ...
  #grid(
    ...
    [Рецензент],
    [#field(
      [#reviewer-position, #reviewer-short],
      w: 100%,
      caption: [(посада, науковий ступінь, вчене звання, прізвище та ініціали)],
    )],
    [#field(none, w: 3.2cm, caption: [(підпис)])],
...
#let task-pages() = [
  ...
  #align(center)[
    ...
    #text(weight: "bold")[на дипломний проєкт студенту]
  ]
  ...
  #grid(
    columns: (auto, 1fr),
    inset: 5pt,
    [1. Тема проєкту#h(0.6em)], [#underline[#title]],
  ...
  затверджені наказом по університету від "3" червня #year р. №2055-с

  2. Термін подання студентом проєкту "14" червня #year року

  3. Вихідні дані до проєкту: #field([технічне завдання])
  ...
  #text(size: 10.5pt)[
    #table(
      ...
      table.header(
        [№\ з/п],
        [#stack(dir: ttb, spacing: 2pt, align(left, [Назва етапів виконання]), align(left, [дипломного проєкту]))],
        [#stack(dir: ttb, spacing: 2pt, align(left, [Термін виконання]), align(left, [етапів проєкту]))],
        [Примітка],
      ),
...
#let annotation-pages() = [
  ...
  Пояснювальна записка дипломного проєкту складається з #pz-section-count() розділів, містить #pz-table-count() таблиць, #pz-image-count() рисунків, #sources-count використаних джерел та додатки — загалом #diploma-pages-by-id("pz") сторінок.

  Дипломний проєкт присвячено розробленню мобільного застосунку ReadMe для перетворення документів довільних форматів на аудіокниги з використанням методів штучного інтелекту. Актуальність роботи зумовлена потребою у зручному прослуховуванні наукових статей, сканованих книг та інших документів зі складною просторовою структурою, для яких традиційний каскад OCR + TTS часто озвучує службові елементи сторінки та не виконує семантичної підготовки тексту до мовлення.

  У роботі проаналізовано предметну область та існуючі програмні рішення, сформульовано функціональні й нефункціональні вимоги, спроєктовано архітектуру системи та реалізовано мобільний клієнт на React Native й Expo. Основою розробки є VLM-centric pipeline, у якому vision-language модель використовується як семантичний шар між OCR-розміткою документа та TTS-синтезом. Реалізовано імпорт документів різних форматів, потокову обробку сторінок, ранній початок прослуховування після готовності першої сторінки, кешування аудіофрагментів, режими перегляду reflowed/original, налаштування голосу й швидкості, пропуск службових блоків, офлайн-завантаження, резервне копіювання бібліотеки та інтеграцію з TTS-провайдерами.
  ...
  Результатом дипломного проєкту є працездатний мобільний застосунок, що забезпечує більш природне перетворення документів на аудіокниги порівняно з прямим озвученням OCR-тексту та може бути використаний для індивідуального прослуховування довгого текстового контенту.
...
#{
  {
    ...
    title-main()
    pagebreak()
    metadata((kind: "diploma-doc-start", id: "task", format: "A4", mark: "", name: "Завдання на дипломний проєкт"))
    task-pages()
